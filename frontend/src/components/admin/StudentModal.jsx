...
// --- SỬA LỖI MẤT FOCUS ---
// Component con FormInput (đã di chuyển ra ngoài)
const FormInput = ({
  label,
  name,
  type = "text",
...
export default function StudentModal({
  ...
  useEffect(() => {
    ...
    if (isEditMode) {
      setFormData({
        ...
        goal: student.goal || "",
        slot_quota: student.slot_quota || 0,
        is_active: student.is_active,
      });
      setAvatarPreview(getAvatarSrc(student.avatar_url, student.email));
      setLiveCertificates(student.certificates || []);
    }
  }, [student, isEditMode]);
  ...
  const handleAddCertificate = async () => {
    ...
    setCertLoading(true);
    try {
      const newCert = await adminApi.addCertificate(student.user_id, {
        title: newCertTitle,
        image_base64: newCertBase64,
      });
      // Cập nhật state cục bộ
      setLiveCertificates((prevCerts) => [...prevCerts, newCert]);
      // Báo cho ListPage tải lại
  ...
  const handleDeleteCertificate = async (certId) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa chứng chỉ này?")) {
      setCertLoading(true);
      try {
        await adminApi.deleteCertificate(student.user_id, certId);
        // Cập nhật state cục bộ
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-between items-center p-4 border-b">
            <h2 className="text-xl font-bold">
              {isEditMode ? "Sửa Học viên" : "Thêm Học viên mới"}
            </h2>
            <button
              type="button"
              onClick={onClose}
                ...
                <div className="flex items-end gap-2">
                  <div className="flex-1">
                    ...
                    <input
                      type="text"
                      value={newCertTitle}
                      onChange={(e) => setNewCertTitle(e.target.value)} // <-- SỬA LỖI TYPO
