...
import { Trash2, PlusCircle, Loader2, AlertCircle, CalendarClock } from "lucide-react";
...
export default function CoachAvailabilityPage() {
  ...
  const handleDeleteRule = async (ruleId) => {
    ...
    try {
      await apiFetch(`/coachers/me/availability/${ruleId}`, { method: "DELETE" });
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-4 sm:p-8">
      ...
      <h1 className="text-3xl font-bold mb-8">Quản lý Lịch rảnh (Availability)</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-6">
            <h2 className="text-2xl font-semibold mb-4">Quy tắc rảnh của bạn</h2>
            <div className="space-y-3">
              {rules.length > 0 ? (
                rules.map((rule) => (
                  <div key={rule.id} className="flex items-center justify-between bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                    <span className="font-mono text-lg font-medium">
                      {WEEKDAYS[rule.weekday]}: {rule.start_time} - {rule.end_time}
                    </span>
                    <button
                ...
                <p className="text-gray-500 text-center py-4">Bạn chưa có quy tắc rảnh nào. Hãy thêm ở dưới.</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-6">
            <h2 className="text-2xl font-semibold mb-4">Thêm quy tắc rảnh mới</h2>
            <form onSubmit={handleAddRule} className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
              <div>
                <label htmlFor="weekday" className="block text-sm font-medium mb-1">Ngày trong tuần</label>
                <select
                  name="weekday" id="weekday"
                  value={newRule.weekday} onChange={handleRuleChange}
              ...
              <div>
                <label htmlFor="start_time" className="block text-sm font-medium mb-1">Từ (Giờ)</label>
                <input
                  type="time" name="start_time" id="start_time"
                  value={newRule.start_time} onChange={handleRuleChange}
                  className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600"
                />
              </div>
              <div>
                <label htmlFor="end_time" className="block text-sm font-medium mb-1">Đến (Giờ)</label>
                <input
                  type="time" name="end_time" id="end_time"
                  value={newRule.end_time} onChange={handleRuleChange}
                  className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600"
                />
              </div>
              <button
              ...
              >
                {savingRule ? <Loader2 className="w-5 h-5 animate-spin"/> : <PlusCircle className="w-5 h-5" />}
        ...
        <div className="bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-6 h-fit">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300 mb-4">
            Dùng các quy tắc rảnh của bạn để tạo ra các lịch trống (OpenSlot) cho học sinh thấy.
          </p>
          <form onSubmit={handleGenerateSlots} className="space-y-4">
             <div>
                <label htmlFor="start_date" className="block text-sm font-medium mb-1">Bắt đầu từ ngày</label>
                <input
                  type="date" name="start_date" id="start_date"
                  value={generateParams.start_date} onChange={handleGenerateChange}
                  className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600"
                />
              </div>
              <div>
                <label htmlFor="days_to_generate" className="block text-sm font-medium mb-1">Tạo cho bao nhiêu ngày (mặc định 14)</label>
                <input
                  type="number" name="days_to_generate" id="days_to_generate"
                  value={generateParams.days_to_generate} onChange={handleGenerateChange}
                  className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600"
                />
              </div>
              <div>
                <label htmlFor="slot_duration_minutes" className="block text-sm font-medium mb-1">Độ dài 1 slot (phút, mặc định 30)</label>
                <input
                  type="number" name="slot_duration_minutes" id="slot_duration_minutes"
                  value={generateParams.slot_duration_minutes} onChange={handleGenerateChange}
                  step="15"
                  className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600"
                />
              </div>
              <button
                type="submit" disabled={generatingSlots}
              ...
              >
                {generatingSlots ? <Loader2 className="w-5 h-5 animate-spin"/> : <CalendarClock className="w-5 h-5" />}
