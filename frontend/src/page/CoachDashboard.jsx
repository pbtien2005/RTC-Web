  ...
  const handleBookingResponse = async (requestId, action) => {
    try {
      ...
      setBookingRequests((prev) =>
        prev.filter((req) => req.id !== requestId) // Xóa yêu cầu đã xử lý
      );
          ...
          <div className="space-y-4">
            {bookingRequests.length > 0 ? (
              bookingRequests.map((book) => (
                <div key={book.id} className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                  <p className="font-semibold">Student ID: {book.student_id}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-300">
                    Lời nhắn: {book.message || "(Không có)"}
                  </p>
                  <p className="text-sm font-medium">Số slot: {book.items.length}</p>
              ...
              <p className="text-gray-500 text-center py-4">Không có yêu cầu đặt lịch nào.</p>
            )}
          </div>
        </div>
          ...
          <div className="space-y-4">
            {chatRequests.length > 0 ? (
              chatRequests.map((chat) => (
                <div key={chat.id} className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                  <p className="font-semibold">Requester ID: {chat.requester_id}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-300">
              ...
              <p className="text-gray-500 text-center py-4">Không có yêu cầu chat nào.</p>
            )}
          </div>
        </div>
