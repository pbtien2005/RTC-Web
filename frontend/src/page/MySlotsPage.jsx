...
export default function MySlotsPage() {
  ...
  const handleSlotSelect = (slot) => {
    ...
    if (slot.status !== 'open' && slot.status !== 'on_hold') {
      showError("Bạn chỉ có thể hủy slot 'open' hoặc 'on_hold'. Slot đã 'booked' phải được hủy qua trang Lịch học.");
      return;
    }
  ...
  const handleCancelSelectedSlots = async () => {
    if (!window.confirm(`Bạn có chắc muốn HỦY ${selectedSlots.length} slot đã chọn?`)) return;
        ...
        {coach && (
          <div className="bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-8 flex flex-col md:flex-row md:items-center md:gap-8">
            ...
            <div className="flex-1 text-center md:text-left mt-6 md:mt-0">
              ...
              <p className="text-xl text-gray-600 dark:text-gray-400">{coach.job}</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-6">
          ...
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-4">
              <button onClick={prevWeek} className="p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700">
                <ChevronLeft size={20} />
              </button>
              <button onClick={nextWeek} className="p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700">
                    ...
                    <div className="flex flex-col gap-2">
                      {slotsForDay.map((slot) => (
                        <button
                          ...
                          disabled={slot.status === 'booked' || slot.status === 'cancelled'}
